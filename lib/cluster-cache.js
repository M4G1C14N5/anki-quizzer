...
//
// File: ${CACHE_DIR}/clusters.json
// Default CACHE_DIR: ${LLM_CACHE_DIR} or ${DATA_DIR}/llm-cache (Coolify volume).
//   In production DATA_DIR=/data so the cache survives container restarts.
...
function defaultCacheDir() {
  if (process.env.LLM_CACHE_DIR) return process.env.LLM_CACHE_DIR;
  if (process.env.DATA_DIR) return path.join(process.env.DATA_DIR, 'llm-cache');
  // Fallback for local dev (no DATA_DIR set): ./data/llm-cache relative to the
  // app root (one level above this file's lib/).
  return path.join(__dirname, '..', 'data', 'llm-cache');
}
